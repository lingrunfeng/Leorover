  ...
  : Node("mtc_grasp_pose_node", options)
{
  ...
  // 顶抓姿态四元数（从 RViz 手动示教获取 2025-12-07）
  this->declare_parameter("top_grasp_orientation", std::vector<double>{0.988, 0.037, 0.021, -0.151});  // x, y, z, w
...
mtc::Task MTCGraspPoseNode::createTask()
{
  ...
  // Pick Object Serial Container
  {
    ...
    // Approach
    {
      ...
      vec.header.frame_id = world_frame;  // 改为世界坐标系
      vec.vector.z = -1.0; // 垂直向下 approach（顶抓）
      stage->setDirection(vec);
      grasp->insert(std::move(stage));
    }

    // Generate Grasp Pose - 固定顶抓姿态
    {
      auto stage = std::make_unique<mtc::stages::GeneratePose>("generate top grasp pose");
      ...
      target_grasp_pose.header.frame_id = world_frame;
      target_grasp_pose.pose = target_pose_.pose;  // 使用物体位置作为基准
      
      // 应用顶抓 orientation（从 RViz 手动示教获取）
      auto top_orientation = this->get_parameter("top_grasp_orientation").as_double_array();
      target_grasp_pose.pose.orientation.x = top_orientation[0];
      target_grasp_pose.pose.orientation.y = top_orientation[1];
      target_grasp_pose.pose.orientation.z = top_orientation[2];
      target_grasp_pose.pose.orientation.w = top_orientation[3];
      
      // 智能高度偏移：根据物体高度和位置自动调整
      double object_height_offset = 0.0;
      auto object_dims = this->get_parameter("object_dimensions").as_double_array();
      auto object_type = this->get_parameter("object_type").as_string();
      
      if (object_type == "cylinder" && object_dims.size() >= 2) {
        // 圆柱体：高度是第一个参数
        object_height_offset = object_dims[0] / 2.0;  // 物体高度的一半
      } else if (object_type == "box" && object_dims.size() >= 3) {
        // 长方体：Z尺寸是第三个参数
        object_height_offset = object_dims[2] / 2.0;
      }
      
      // Grasp pose设在物体顶部上方5cm处（从10cm减小到5cm以适应低物体）
      // 对于z很小的物体，假设物体底部在地面，中心应该在 z + object_height_offset
      double grasp_approach_offset = 0.05;  // 从顶部再往上5cm
      target_grasp_pose.pose.position.z += object_height_offset + grasp_approach_offset;
      
      RCLCPP_INFO(this->get_logger(), 
                  "Grasp pose: object_z=%.3f, height_offset=%.3f, approach_offset=%.3f, final_z=%.3f",
                  target_pose_.pose.position.z, object_height_offset, grasp_approach_offset,
                  target_grasp_pose.pose.position.z);
      
      stage->setPose(target_grasp_pose);

      auto wrapper = std::make_unique<mtc::stages::ComputeIK>("grasp pose IK", std::move(stage));
      wrapper->setMaxIKSolutions(20);
      wrapper->setMinSolutionDistance(1.0);
      
      // 顶抓：使用单位变换，因为 target_grasp_pose 已包含完整的末端执行器姿态
      // 不需要额外的 grasp_frame_transform（那是为侧抓设计的）
      Eigen::Isometry3d identity_transform = Eigen::Isometry3d::Identity();
      wrapper->setIKFrame(identity_transform, gripper_frame);
...
void MTCGraspPoseNode::resetAfterGrasp()
{
  ...
  try {
    ...
    // Step 5: Reset gripper to OPEN (匹配MTC task的初始状态)
    RCLCPP_INFO(this->get_logger(), "Resetting gripper to open...");
    auto gripper_open_pose = this->get_parameter("gripper_open_pose").as_string();
    gripper_group->setNamedTarget(gripper_open_pose);
